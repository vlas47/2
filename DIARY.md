...
- Полностью заменил `soyz.html` на новый фасад по макету: обновлённый хедер, герой, секции о компании/каталоге/услугах/преимуществах/отзывах/новостях/партнёрах, заявка и футер.
- Подключил новые стили, шрифты Viaoda Libre и Montserrat, добавил адаптивные сетки и лёгкие анимации появления.
- Подгрузил экспортированные изображения макета в `static/landing` и привязал их к новым блокам.
...
- Починил публичные карточки ЖК: заменил `<slug:slug>` на `<str:slug>` в `propertysearch/urls.py`, чтобы русские slug открывались, выложил файл на сервер и перезапустил `realty.service`.
- На детальной странице ЖК превью внизу стали переключать главное фото: клики по миниатюрам подставляют выбранный кадр в верхний баннер.
- Расширил карточки предложений под страницей ЖК: выводим компактный набор ключевых параметров (ID, цена, планировка, этаж, адрес, метро, тип/статус) без фотографий.
- Упростил карточки предложений: убрал ID/Case/дату обновления, добавил цветной фон и лёгкую анимацию подъёма при наведении; оставил только флаги типа объекта и ключевые параметры.
- Исправил фильтр по комнатам в публичной карточке ЖК: запросы с `rooms=` больше не падают на отсутствующее поле `room_count`.
- Расширил фильтры на деталке ЖК: комнаты теперь чекбоксами (можно выбирать несколько), добавлены сортировки (цена/площадь/этаж вверх-вниз) и сохранён фильтр по цене.
- Убрал дубли в поле тип/категория, перевёл `unfinished` в «Строится», добавил вывод времени до метро (пешком/транспорт) в карточках предложений.
- Добавил больше параметров поиска на деталке ЖК: площади, этажность, флаги новостройка/апартаменты/студия и «только с отделкой».
- Перекомпоновал фильтры на деталке ЖК в два блока: тип/комнаты/этаж/флаги и цена/площадь/сортировка, сделали компактную сетку.
- Добавил разворачиваемые карточки квартир на странице ЖК: кнопка «Подробнее» раскрывает описание, параметры сделки и здания; по умолчанию первая карточка открыта.
## 2026-01-20

- Переверстал главную `soyz.html` по новому макету Figma: обновил структуру секций, сетку, размеры, фоны/декор и стили.
- Настроил адаптивы под 830/650/360 и оставил без изменений функциональные ссылки и метрику.
=======
